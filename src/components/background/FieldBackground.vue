...
class Cloud {
 ...
 this.speed = (Math.random() * 0.05 + 0.015) * (Math.random() > 0.5 ? 1 : -1);
...
const drawCloud = (ctx, cloud) => {
 ...
 const gradient = ctx.createRadialGradient(cloud.x + cloud.width * 0.5, cloud.y + cloud.height * 0.5, 0, cloud.x + cloud.width * 0.5, cloud.y + cloud.height * 0.5, cloud.width * 0.5);
 if (currentTheme.value) {
 ...
 ctx.fillStyle = gradient;
 ctx.beginPath();
 ctx.ellipse(cloud.x + cloud.width * 0.5, cloud.y + cloud.height * 0.5, cloud.width * 0.5, cloud.height * 0.55, 0, 0, Math.PI * 2);
 ctx.fill();
 ctx.restore();
};
const drawGrass = (ctx, canvas) => {
 ...
 // 主草地渐变
 const grassGradient = ctx.createLinearGradient(0, grassStartY, 0, canvas.height);
 if (currentTheme.value) {
 // 暗色模式 - 暗夜草地
 ...
 layer.update(time);
 const layerGradient = ctx.createLinearGradient(0, grassStartY, 0, canvas.height);
 ...
 const wave = Math.sin(x * 0.0015 + layer.offset) * 6 + Math.sin(x * 0.0025 + layer.offset * 0.8) * 3;
...
const drawParticle = (ctx, particle) => {
 ...
 const color = particle.colorType === 'warm'
 ? { r: 255, g: 230, b: 120 }
 : { r: 180, g: 255, b: 200 };
 ctx.save();
...
const drawWarmOverlay = (ctx, canvas) => {
 const gradient = ctx.createRadialGradient(canvas.width * 0.7, canvas.height * 0.55, 0, canvas.width * 0.7, canvas.height * 0.55, canvas.width * 0.7);
 if (currentTheme.value) {
