...
class Particle {
 ...
 // 气泡数量
 bubbles.value = [];
 const bubbleCount = Math.min(Math.floor((canvas.width * canvas.height) / 8000), 30);
...
const drawLightBeams = (ctx, canvas) => {
 ...
 const opacity = currentTheme.value ? (0.015 + Math.sin(time * 0.2 + i * 0.4) * 0.006) : (0.025 + Math.sin(time * 0.2 + i * 0.4) * 0.01);
...
const drawBubble = (ctx, bubble) => {
 ...
 ctx.beginPath();
 ctx.arc(screenX - size * 0.2, screenY - size * 0.2, size * 0.2, 0, Math.PI * 2);
 ctx.fill();
 }
 ctx.restore();
};
