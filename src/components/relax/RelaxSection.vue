<script setup>
  ...
  import {
    fetchHistoryEvents as apiFetchHistoryEvents,
    fetchCalendarDay,
  } from '../../utils/api';
      ...
      <div class="relax-main">
        ...
        <div class="date-column">
            ...
            <div class="date-display">
              ...
              <div class="date-weekday-row">
                <span class="date-weekday">{{
                  calendarData?.week || today.weekDay
                }}</span>
                <span v-if="calendarData" class="weekday-divider">|</span>
                <div v-if="calendarData" class="calendar-details">
                  <span v-if="calendarData.gzYear" class="calendar-item">{{
                    calendarData.gzYear
                  }}</span>
                  <span
                    v-if="calendarData.gzYear && calendarData.gzMonth"
                    class="calendar-divider"
                    >·</span
                  >
                  <span v-if="calendarData.gzMonth" class="calendar-item">{{
                    calendarData.gzMonth
                  }}</span>
                  <span
                    v-if="
                      (calendarData.gzMonth || calendarData.gzYear) &&
                      calendarData.gzDate
                    "
                    class="calendar-divider"
                    >·</span
                  >
                  <span v-if="calendarData.gzDate" class="calendar-item">{{
                    calendarData.gzDate
                  }}</span>
                  <span
                    v-if="
                      (calendarData.gzDate ||
                        calendarData.gzMonth ||
                        calendarData.gzYear) &&
                      (calendarData.lMonth || calendarData.lDate)
                    "
                    class="calendar-divider"
                    >·</span
                  >
                  <span
                    v-if="calendarData.lMonth && calendarData.lDate"
                    class="calendar-item"
                    >农历 {{ calendarData.lMonth }}月{{
                      calendarData.lDate
                    }}</span
                  >
                </div>
          ...
          <div class="history-area">
            ...
            <div ref="historyListRef" class="history-list custom-scrollbar">
              ...
              <template v-else>
                ...
                <div
                  v-for="(event, index) in funnyEvents"
                  :key="'funny-' + index"
                  class="history-item funny-item"
                >
                  <span class="history-year">{{ event.year }}年</span>
                  <span class="history-dot">·</span>
                  <span class="history-event">{{ event.title }}</span>
          ...
          <div class="disclaimer">
            <span
              >内容来自第三方接口，可能存在误差，仅供参考，不构成史实依据</span
            >
          </div>
        </div>
      </div>
